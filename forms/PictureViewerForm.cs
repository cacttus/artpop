...
namespace ArtPop
{
   public partial class PictureViewerForm : MonoEditForm
   {
      ...
      public void SetImage(string imageLoc)
      {
         ...
         try
         {
            if (String.IsNullOrEmpty(imageLoc) == false)
            {
               ...
               _btnExclude.Visible = _btnFavorite.Visible = true;

            }
            else
            {
               _pbImage.Image = null;
               _btnExclude.Visible = _btnFavorite.Visible = false;
            }
         }
         catch (Exception ex)
         {
      ...
      private void PictureViewer_Load(object sender, EventArgs e)
      {
         ...
         Globals.SetTooltip(_btnPlay, new Phrase("Play/Pause (F5)"));
         Globals.SetTooltip(_btnPrevious, new Phrase("Previous Image (Left)"));
         Globals.SetTooltip(_btnNext, new Phrase("Next Image (Right)"));
         Globals.SetTooltip(_btnOpenImageInFolder, new Phrase("Open image in folder"));
         Globals.SetTooltip(_btnRestore, new Phrase("Fullscreen/Window (F11)"));
         Globals.SetTooltip(_btnFavorite, new Phrase("Add Favorite (F)"));
         Globals.SetTooltip(_btnExclude, new Phrase("Exclude From Deck (E)"));
      }
      ...
      private void _btnOpenImageInFolder_Click(object sender, EventArgs e)
      {
         try
         {
            if (_strImageLoc != null)
            {
               string dir = System.IO.Path.GetDirectoryName(System.IO.Path.GetFullPath(_strImageLoc));
               string file = System.IO.Path.GetFileName(_strImageLoc);
               Globals.OpenFolderAndSelectItem(dir,file);
            }
         }
         catch (Exception ex)
         {
      ...
      private void _txtInstructions_TextChanged(object sender, EventArgs e)
      {

      }

      private void _btnExclude_Click(object sender, EventArgs e)
      {
         Globals.MainForm.Sequencer.ExcludeCurrentCard();
      }
   }
}
