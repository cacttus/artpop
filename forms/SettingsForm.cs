...
namespace ArtPop
{
   public partial class SettingsForm : MonoEditForm
   {
      ...
      public void RefreshFileCache()
      {
         ...
         else
         {
            ...
            FileCache.Search(_txtSearchPath.Text, filter, (value01) =>
            {
                   //Progress Action
                   _pbSearchProgress.BeginInvoke(
                       (Action)(() =>
                       {
                       _pbSearchProgress.Value = (int)((double)_pbSearchProgress.Maximum * value01);
                    })
                   );
            }, () =>
            {
               ...
               MarkChanged();
               _btnSearch.Text = "Search";
               _pbSearchProgress.Value = _pbSearchProgress.Maximum;
            });
            _btnSearch.Text = "Cancel Search";
         }
      }
      public void UpdateLog(string st)
      {
         _txtLog.Text = st;
      ...
      private void SettingsForm_Load(object sender, EventArgs e)
      {
         ...
         Globals.SetTooltip(_btnSave, new Phrase("Save", "Salvar"));
         Globals.SetTooltip(_btnSearch, new Phrase("Search", "Encontrar"));
         Globals.SetTooltip(new List<Control>() { _lblFullscreenModeEnabled, _chkFullscreenModeEnabled },
             new Phrase("Pictures will fill the screen.", "Las imágenes llenarán la pantalla."));
         Globals.SetTooltip(new List<Control>() { _lblShowDetailsInFullscreenMode, _chkShowDetailsInFullscreenMode },
             new Phrase("Image details visible in fullscreen mode.", "Detalles de la imagen visibles en modo de pantalla completa."));
         Globals.SetTooltip(_btnSelectSearchPath, new Phrase("Browse..", "Vistazo.."));

         string cpath = Globals.GetAssemblyDirectory();
         string cpath_en = "App Path: " + cpath;
         string cpath_esp = "Ruta de aplicación" + cpath;
         Globals.SetTooltip(new List<Control>() { _txtSearch, _txtSearchPath }, new Phrase(cpath_en, cpath_esp));
      }
      ...
      private void _btnSelectSearchPath_Click(object sender, EventArgs e)
      {
         ...
         try
         {
            curdir = Globals.GetAssemblyDirectory();
         }
         catch
         {
            curdir = Environment.GetFolderPath(Environment.SpecialFolder.Desktop);
         }
         string[] folder = Globals.GetValidUserFolder(curdir, false);
         if (folder.Length == 1)
         {
